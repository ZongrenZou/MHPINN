...
class Coupling(tf.keras.layers.Layer):
    ...
    def __init__(self, dim, num_masked, hidden_layers, activation=tf.nn.relu):
        ...
        self.nn += [tf.keras.layers.Dense(2 * (self.dim - self.num_masked))]
...
class Flow(tf.keras.Model):
    ...
    def __init__(self, dim, permutation, hidden_layers, num_bijectors, activation, opt, eps, name):
        super().__init__()
        self.dim = dim
...
class RealNVP(Flow):
    ...
    def __init__(
        ...
        super().__init__(dim, permutation, hidden_layers, num_bijectors, activation, opt, eps, name)
        bijectors = []
        for i in range(self.num_bijectors):
            bijectors += [
                tfb.RealNVP(
                    num_masked=num_masked,
                    shift_and_log_scale_fn=Coupling(
                        dim=self.dim, 
                        num_masked=num_masked, 
