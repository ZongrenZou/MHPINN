...
class PI_Bayesian:
    def __init__(
        self, inputs_u, u, inputs_f, f, pde_fn, meta, flow, noise_u=0.05, noise_f=0.05,
    ):
        self.inputs_u = inputs_u
        self.u = u
    ...
    def build_posterior(self):
        ...
        def _fn(*variables):
            ...
            xi = variables[0]
            head = tf.transpose(xi)

            inputs_u = tf.constant(self.inputs_u, dtype=tf.float32)
            inputs_f = tf.constant(self.inputs_f, dtype=tf.float32)

            u_pred = self.meta.call(inputs_u, head)
            f_pred = self.pde_fn(inputs_f, self.meta.call(inputs_f, head))
